function Analysis4_meanOfRatings_reduced(resDir)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Get the average of the ratings 
% This is happening because we wanted to merge together features that are
% highly correlated (Section S.2.2.3)
...
upperLimbs = mean(T(1:2,:));
hands = mean(T(3:5,:));
lowerLimbs = mean(T(6:8,:));
TT = [upperLimbs; hands; lowerLimbs; T(9:end,:)]; 
T = TT;

% save it (for 'radarPlots_fromSingleRatings.m')
TT_table = array2table(T);
pathAndName = fullfile(resDir,'normalized_featureRatings_part1_reduced.xlsx');
...
DailyRoutine = [5,6,74,77,85,95,100];
FoodRelated = [10,14,16,21,25,26,31,51,63,80,94];
Sport = [3,7,12,27,30,34,35,37,40,42,44,45,46,47,52,58,59,61,65,70,71,72,73,75,76,78,82,83,84,89];
HouseholdActivities = [9,11,13,15,18,19,28,29,32,41,55,67,68,93];
SocialInteraction = [36,38,43,57];
Communication =[8,69,88,91,98];
HandMovement = [1,33,50,54,64,90,97,99];
Locomotion = [22,23];
Aggression = [2,4,66,87];
UnnamedClust1 = [39,49,62];
...
pathAndName = fullfile(resDir,'averaged_featureRatings_part1_reduced.xlsx');
writetable(TT_table,pathAndName);
